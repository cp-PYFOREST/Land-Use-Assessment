...
## INFONA Forest Cover Raster

```{r}
# Forest cover --------------------------------------------------------
fc_20 <- terra::rast(file.path(datadir, "fc_infona/cf_rast_20.tif"))
```
...
```{r}
#transform the CRS of lup to the the forest cover raster (fc_20)
lup_proj <- st_transform(lup, crs(fc_20))
```
...
```{r}
# Check the extent of forest cover raster and filtered lup data
ext(fc_20)
st_bbox(lup_active_year)
```

```{r}
# Update the extent of the forest cover raster to match the extent of the filtered lup data
...
```{r}
tic()
auth_year_touches <- terra::extract(fc_20, lup_active_year, fun=sum, na.rm=TRUE, ID=TRUE, touches = TRUE, bind = TRUE)
toc()
```
