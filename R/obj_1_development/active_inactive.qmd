...
limit_year <- limit_dup |>
  filter(anho_capa <= 2020 & year_inactive >= 2020) |>
  mutate(fecha_res = as.Date(fecha_res))

st_write(limit_year,'/Users/romero61/../../capstone/pyforest/data/active_inactive/limit_year/limit_year.shp')

```

```{r}
limit_year_intersection <- st_intersection(limit_year, limit_dup)
```
...
lim_put <- lim_put %>%
  mutate(fecha_res = as.Date(fecha_res))%>%
  group_by(codigo_put) %>%
  mutate(duplicate = ifelse(n() == 1, 0, 1)) %>%
  ungroup() %>%
  group_by(codigo_put) %>%
  mutate(ultimo = ifelse(fecha_res == max(fecha_res), 1, 0)) %>%
  ungroup()%>%
  mutate(act_2023 = case_when(estado == 0 ~ 0,
                               estado == 1 & duplicate == 1  &  ultimo ==1 ~ 1,
                               estado == 1 & duplicate == 0 ~ 1,
                               TRUE ~ 0))
```
...
```{r}
#st_write(active_intersection,'/Users/romero61/../../capstone/pyforest/data/active_inactive/active_inactive_temp/active_inactive_intersection.gpkg')
```
...
#st_write(active_inactive,'/Users/romero61/../../capstone/pyforest/data/active_inactive/active_inactive_temp/active_inactive1920.gpkg')
```
...
#st_write(clean,'/Users/romero61/../../capstone/pyforest/data/active_inactive/active_inactive_temp/clean1920.gpkg')
```
